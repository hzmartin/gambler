...
public abstract class AbstractCommand implements ICommand {
	...
	public boolean isSubCommand(String subCmd) {
		return params != null && params.length >= 1 && subCmd.equalsIgnoreCase(params[0]);
	}

	/**
	 * compare the params[paramPos] with "go" "go" only can put on the final
	 * position in the params
	 * 
	 * @param paramPos
	 * @return true: dry run
	 */
